...
public sealed class CustomTestCraftDriverTests(CustomFixture fixture) : IClassFixture<CustomFixture> {
    ...
    public async Task ExecutarEmTransacaoAsync_Confirma_QuandoSemExcecao() {
        ...
        await _driver.ExecuteInTransactionAsync(async (drv, tx, ct) =>
            await drv.ExecuteAsync(new Query("tx_test_async").AsInsert(new { name = "TxCommitAsync" }), tx, ct), ct: TestContext.Current.CancellationToken);
    ...
    public async Task ConsultarAsync_LancaELogaErro_QuandoTabelaNaoExiste() {
        ...
        var action = async () => await _driver.QueryAsync<Person>(query, ct: TestContext.Current.CancellationToken);
    ...
    public async Task ExecutarAsync_LancaELogaErro_QuandoSqlInvalido() {
        var action = async () => await _driver.ExecuteAsync("INVALID SQL COMMAND", ct: TestContext.Current.CancellationToken);
        await action.Should().ThrowAsync<Exception>();
    }
    ...
    public async Task DisposeConexaoAsync_Chamado_EmConsultaAsync() {
        var driver = CriarDriver();
        var action = async () => await driver.QueryAsync<Person>(new Query("table_inexistente_async"), ct: TestContext.Current.CancellationToken);
        await action.Should().ThrowAsync<Exception>();
    ...
    public async Task LogErroAsync_Chamado_EmExcecaoConsultaAsync() {
        ...
        var query = new Query("table_inexistente_async").Select("id", "name");
        var action = async () => await driver.QueryAsync<Person>(query, ct: TestContext.Current.CancellationToken);
        await action.Should().ThrowAsync<Exception>();
        logger.ErrorCalled.Should().BeTrue();
        logger.LastException.Should().NotBeNull();
        logger.LastSql.Should().NotBeNull();
    ...
    public async Task ConsultarPrimeiroOuPadrao_Deve_LogarErro_EmExcecao() {
        ...
        var action5 = async () => await driver.QueryFirstOrDefaultAsync<object>(query, ct: TestContext.Current.CancellationToken);
        await action5.Should().ThrowAsync<InvalidOperationException>();

        var action6 = async () => await driver.QueryAsync<object>(query, ct: TestContext.Current.CancellationToken);
        await action6.Should().ThrowAsync<InvalidOperationException>();

        var action7 = async () => await driver.ExecuteAsync(query, ct: TestContext.Current.CancellationToken);
        await action7.Should().ThrowAsync<InvalidOperationException>();

        var action8 = async () => await driver.ExecuteAsync("SELECT 1", ct: TestContext.Current.CancellationToken);

...
public sealed class CustomTestCraftDriver(string conn, IDatabaseLogger? logger = null)
    : SQLCraftDriver(conn, logger)
{
    ...
    protected override async Task DisposeConnAsync(DbConnection? conn)
    {
        ...
        await base.DisposeConnAsync(conn).ConfigureAwait(false);
    }

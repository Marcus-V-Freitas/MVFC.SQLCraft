...
public sealed class CustomTestQueryFactory(IDbConnection conn, Compiler compiler, CustomTestCraftDriver driver) : IQueryFactory
{
    ...
    public async Task<T?> FirstOrDefaultAsync<T>(Query query, IDbTransaction? transaction = null, int? timeout = null, CancellationToken cancellationToken = default) =>
        _driver.ThrowInternalError
            ...
            : await _queryFactory.FirstOrDefaultAsync<T>(query, transaction, timeout, cancellationToken).ConfigureAwait(false);
    ...
    public async Task<IEnumerable<T>> GetAsync<T>(Query query, IDbTransaction? transaction = null, int? timeout = null, CancellationToken cancellationToken = default) =>
        _driver.ThrowInternalError
            ...
            : await _queryFactory.GetAsync<T>(query, transaction, timeout, cancellationToken).ConfigureAwait(false);
    ...
    public async Task<int> ExecuteAsync(Query query, IDbTransaction? transaction = null, int? timeout = null, CancellationToken cancellationToken = default) =>
        _driver.ThrowInternalError
            ...
            : await _queryFactory.ExecuteAsync(query, transaction, timeout, cancellationToken).ConfigureAwait(false);
